...
case $OPTION in
    ...
    6) set_dns ;;
    7) 
       echo -e "${GREEN}正在更新...${PLAIN}"
       if command -v systemctl &>/dev/null; then systemctl stop hysteria-server; else rc-service hysteria-server stop; fi
       bash <(curl -fsSL https://get.hy2.sh/)
       if command -v systemctl &>/dev/null; then systemctl restart hysteria-server; else rc-service hysteria-server restart; fi
       echo -e "${GREEN}更新完毕${PLAIN}"
       ;;
    8)
        echo -e "${GREEN}>>> 正在更新管理脚本...${PLAIN}"
        
        # 强制指定下载到 /root 目录，这是 Linux VPS 的标准目录
        ...
        # 重要：运行新下载的脚本！
        # 这会触发新脚本里的 create_shortcut，从而更新 /usr/bin/hy
        bash "$TARGET_FILE"
...
function start_menu(){
    ...
    case $k in
        ...
        2) 
           ...
           ;;
        3) 
           if [ -f "/usr/bin/hy" ]; then hy; else echo "未安装"; fi 
           ;;
        0) exit 0 ;;
        *) echo "错误" ;;
    esac
